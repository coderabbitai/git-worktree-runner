...
- `docs/troubleshooting.md` replaces a `bash -x git gtr` instruction that cannot work with a trace of the real script.
...
- `AGENTS.md` and `CLAUDE.md` attributed `postCd` hooks to `run_hooks_in`; they now describe `run_hooks_export` and the `init`-generated shell functions, and note that only the AI-tool path runs them.
