...
- `.github/copilot-instructions.md` rewritten for the current `bin/git-gtr` and `lib/` layout; it had described a single-file `bin/gtr` with an `open` command since before the modularization.
...
- The same guidance now describes the `lib/hooks.sh` dispatchers accurately, distinguishes the `postCd` paths (the AI launch path runs them when the tool starts, while the shell functions generated by `init` run them for `gtr cd` and the `--cd` flows), and records that `git gtr completion zsh` emits an fpath snippet rather than the generated file.
